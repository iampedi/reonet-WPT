...
<style>
body { font-family: Arial, sans-serif; background: #f3f3f3; margin: 0; padding-top: 60px; }
...
#upload-container { max-width:500px; margin:40px auto; padding:25px; background:#fff; border-radius:12px; display:none; }
#before-list img, #after-list img { width:120px; border-radius:8px; cursor:pointer; display:block; margin-bottom:5px; }
...
.delete-btn { display:block; margin-top:5px; color:red; cursor:pointer; font-size:12px; }
	/* ===== Mobile friendly ===== */
#upload-container {
    margin: 15px;
    padding: 15px;
...
jQuery(document).ready(function(){
    ...
    if(token && is_admin){ jQuery('#login-form-container').hide(); jQuery('#upload-container').show(); jQuery('#logout-btn').show(); }
    ...
    jQuery('#custom-login-form').on('submit', function(e){
        ...
        let btn = jQuery('#login-btn'); btn.prop('disabled', true); btn.html('Login... <span class="loading-spinner"></span>');
        ...
        jQuery.ajax({
            ...
            success: function(res){
                if(res.token){
                    if(!res.is_admin || res.is_admin===0){ jQuery('#login-message').text("Access Denied"); btn.prop('disabled',false); btn.html('Login'); return; }
                    document.cookie = "auth_token="+res.token+"; path=/; max-age=3600";
                    localStorage.setItem('is_admin',res.is_admin);
                    jQuery('#login-form-container').hide(); jQuery('#upload-container').show(); jQuery('#logout-btn').show();
                    btn.prop('disabled',false); btn.html('Login');
                } else { jQuery('#login-message').text(res.message || 'Invalid Email or Password'); btn.prop('disabled',false); btn.html('Login'); }
            },
            error: function(xhr){ let err="Server Connection Error"; if(xhr.responseJSON && xhr.responseJSON.error){ err=xhr.responseJSON.error } jQuery('#login-message').text(err); btn.prop('disabled',false); btn.html('Login'); }
        });
    });
});
...
function generateFileName(barcode,index){ return `${barcode}_${Date.now()}_${index}.jpg`; }
...
async function loadOrderDetailAndImages(barcode){
    ...
    try{
        const res=await fetch(`https://reonet-api.onrender.com/api/Orders/get-detail-by-barcode?barcode=${barcode}`, { headers:{ "Authorization":"Bearer "+token } });
        if(!res.ok){ jQuery("#status").html(`<span style="color:red">Invalid barcode or no data found</span>`); return; }
    ...
    } catch(err){ console.error(err); jQuery("#status").html(`<span style="color:red">Error loading data</span>`); }
}
...
document.getElementById("uploadBtn").onclick=async function(){
    ...
    const statusBox=document.getElementById("status"); statusBox.innerHTML="";
    for(let i=0;i<files.length;i++){
        ...
        } catch(err){ console.error(err); statusBox.innerHTML+=`<p style="color:red">Upload Failed for image ${i+1}</p>`; }
    }
