...
<div class="woocommerce-shipping-totals shipping flex flex-col gap-2 px-5 py-3">
	...
	<div class="space-y-3">
		<?php if (! empty($available_methods) && is_array($available_methods)) : ?>
			<ul id="shipping_method" class="woocommerce-shipping-methods space-y-1.5">
				<?php foreach ($available_methods as $method) : ?>
					<li class="flex items-center gap-2">
						...
						printf('<label class="flex items-center justify-between w-full" for="shipping_method_%1$s_%2$s">%3$s</label>', $index, esc_attr(sanitize_title($method->id)), wc_cart_totals_shipping_method_label($method)); // WPCS: XSS ok.
		...
		elseif (! $has_calculated_shipping || ! $formatted_destination) :
			if (is_cart() && 'no' === get_option('woocommerce_enable_shipping_calc')) {
				echo '<p class="woocommerce-shipping-message text-gray-600">' . wp_kses_post(apply_filters('woocommerce_shipping_not_enabled_on_cart_html', __('Enter your address to view shipping options.', 'woocommerce'))) . '</p>';
			} else {
