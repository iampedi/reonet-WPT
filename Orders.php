...
<head>
    ...
    <style>
.status-1 { background-color:#ecf0f1 !important; } /* Order Received */
.status-2 { background-color:#d6eaff !important; } /* Washing */
.status-3 { background-color:#ffe0b2 !important; } /* Drying */
.status-4 { background-color:#d4edda !important; } /* Ready */
.status-5 { background-color:#1f6f28 !important; color:#fff !important; } /* Delivered */
.child-content {
    padding: 10px;
    background: #f9f9f9;
    border-radius: 6px;
...
@media (max-width: 768px) {
    .child-content {
        max-width: 100px !important; /* کمی فاصله از لبه‌ها */
        margin: auto;
    }
	}
.splide {
    ...
    cursor: zoom-out;
		...
		.splide__arrow {
    background-color: #268B32 !important; /* سبز */
    color: #fff !important; /* فلش‌ها سفید */
    border-radius: 4px;
    width: 30px;
    height: 30px;
}
...
/* هاور روی دکمه‌ها */
.splide__arrow:hover {
    background-color: #1f6f28 !important; /* سبز تیره‌تر */
}

/* نقاط پایینی */
.splide__pagination__page {
    background-color: #268B32 !important; /* نقطه فعال */
    opacity: 0.5; /* نقطه غیر فعال کم‌رنگ */
}
...
@media (max-width: 768px) {
    .order-image-slide {
        max-width: 50px;   /* تقریباً عرض کل صفحه */
        max-height: 250px;
    }
}
...
@keyframes spin {
    ...
    background: #268B32; /* سبز مثل دکمه فیلتر */
    color: #fff;
    border: none;
    padding: 6px 12px;
    border-radius: 4px;
    cursor: pointer;
    margin-left: 10px; /* فاصله از دکمه فیلتر */
}
.dt-button.custom-print-btn:hover {
    background: #1f6f28; /* هاور سبز تیره */
}
        ...
        .login-container{
            max-width: 400px;
            margin: 80px auto 20px; /* فاصله از هدر */
            padding: 20px;
            background: #ffffff;
            border-radius: 12px;
        ...
        .login-container button, .login-container #filter-orders {
        ...
        #orders-filter {
            margin-bottom: 20px; /* فاصله بین تاریخ و جدول */
        }
        table.dataTable {
            width: 100% !important;
        }
...
#orders-table tfoot th, #orders-table tfoot td {
    text-align: left !important;
}
...
<script src="https://cdn.jsdelivr.net/npm/glightbox/dist/js/glightbox.min.js"></script>
	...
	<script>
        ...
        function setDefaultDates() {
            ...
            return { start: formatDateCustom(firstDay), end: formatDateCustom(lastDay) };
        }
...
var table = $('#orders-table').DataTable({
    ...
    initComplete: function() {
        ...
        $('.dt-button.custom-print-btn').css({
            ...
            function(){ $(this).css('background','#1f6f28'); },
            function(){ $(this).css('background','#268B32'); }
        );
    },
...
setTimeout(() => {
    ...
    if (beforeEl) new Splide(beforeEl, { type:'loop', perPage:1, height:250 }).mount();
    if (afterEl)  new Splide(afterEl,  { type:'loop', perPage:1, height:250 }).mount();
}, 50);
