...
<div class="component contact">
    <div class="container px-3 sm:px-0">
        <div class="grid sm:grid-cols-2 mt-4 sm:mt-8 gap-6 sm:gap-14">
            ...
            <div class="contact-info px-8 py-8 sm:px-16 bg-[#44a159] rounded-3xl text-white">
                ...
                <h2 class="text-2xl font-medium my-4"><?php reonet_tr('Customer Service', 'Asiakaspalvelu') ?></h2>
                <div class="_contact-det mb-6 flex flex-col gap-4 text-lg">
                    <div>
                        <div class="flex items-center gap-1">
                            <h3 class="font-semibold"><?php reonet_tr('Phone:', 'Puhelin:') ?></h3>
                            <span><?php echo get_field('phone', 'option'); ?></span>
                        </div>
                        <div class="flex items-center gap-1">
                            <h3 class="font-semibold"><?php reonet_tr('Email:', 'Sähköposti:') ?></h3>
                            <span><?php echo get_field('mail', 'option'); ?></span>
                        </div>
                    </div>
                    <div>
                        <h3 class="font-semibold"><?php reonet_tr('Opening Hours:', 'Aukioloajat:') ?></h3>
                        <?php echo get_field('hours', 'option'); ?>
                    </div>
                    <div>
                        <h3 class="font-semibold"><?php reonet_tr('Store Address:', 'Myymälän Osoite:') ?></h3>
                        <div>
                            <span><?php reonet_tr('Address 1:', 'Osoite 1:') ?></span>
                            <span> <?php echo get_field('address_store', 'option'); ?></span>
                        </div>
                        <div>
                            <span><?php reonet_tr('Address 2:', 'Osoite 2:') ?></span>
    ...
    <script>
        ...
        }).setView(map1Cords, 13);
        L.marker(map1Cords, {
            icon: greenIcon
        }).addTo(map1).bindPopup('<?php the_field('map_1_badge', 'option'); ?>');
        ...
        var map2 = L.map('map2', {
            scrollWheelZoom: false
        }).setView(map2Cords, 13);
        L.marker(map2Cords, {
            icon: greenIcon
        }).addTo(map2).bindPopup('<?php the_field('map_2_badge', 'option'); ?>');
        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 19,
            attribution: '© OpenStreetMap',
        }).addTo(map2);
